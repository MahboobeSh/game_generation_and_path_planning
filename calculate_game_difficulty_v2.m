function difficulty_metrics = calculate_game_difficulty_v2(path, obstacles, obstacle_radii, X_s, X_e, x_range, y_range, number_of_pairs, Start_points, End_points, num_samples_list)
    ...
    norms.goal_alignment = min(rehab_metrics.GoalAlignmentAvgDeg / 90, 1);  % Normalize (90 deg = 1)
    norms.y_reversals = min(rehab_metrics.Y_Reversals / 5, 1);  % Normalize
    norms.verticality = min(rehab_metrics.Verticality / 2, 1);  % Normalize
    norms.total_rotation = min(rehab_metrics.TotalRotation / 360, 1);  % Normalize (360 deg = 1)
    norms.gate_rotation = min(rehab_metrics.GateRotation / 180, 1);  % Normalize (180 deg = 1)
    ...
    weights.goal_alignment = 1;
    weights.y_reversals = 1;
    weights.verticality = 1;
    weights.total_rotation = 1;
    weights.gate_rotation = 1;
    ...
    total_weight = weights.path_length + weights.curvature + weights.slope_variance + ...
                   ...
                   weights.inflection_counts + weights.goal_alignment + weights.y_reversals + ...
                   weights.verticality + weights.total_rotation + weights.gate_rotation + ...
                   weights.rehab_composite;
    ...
    weights.goal_alignment = weights.goal_alignment / total_weight;
    weights.y_reversals = weights.y_reversals / total_weight;
    weights.verticality = weights.verticality / total_weight;
    weights.total_rotation = weights.total_rotation / total_weight;
    weights.gate_rotation = weights.gate_rotation / total_weight;
    ...
    overall_difficulty = ...
        ...
        weights.goal_alignment * norms.goal_alignment + ...
        weights.y_reversals * norms.y_reversals + ...
        weights.verticality * norms.verticality + ...
        weights.total_rotation * norms.total_rotation + ...
        weights.gate_rotation * norms.gate_rotation + ...
    ...
    difficulty_metrics.goal_alignment_avg_deg = rehab_metrics.GoalAlignmentAvgDeg;
    difficulty_metrics.y_reversals = rehab_metrics.Y_Reversals;
    difficulty_metrics.verticality = rehab_metrics.Verticality;
    difficulty_metrics.total_rotation = rehab_metrics.TotalRotation;
    difficulty_metrics.gate_rotation = rehab_metrics.GateRotation;
...
function metrics = create_empty_metrics()
    ...
    metrics.goal_alignment_avg_deg = 0;
    metrics.y_reversals = 0;
    metrics.verticality = 0;
    metrics.total_rotation = 0;
    metrics.gate_rotation = 0;
